...
import { Truck, Printer, RefreshCw, Maximize2, Minimize2, ArrowUpRight, FileCheck, Layers } from "lucide-react"
...
export function SkyCmrView() {
  ...
  return (
    <div className={`w-full overflow-hidden bg-slate-950 flex flex-col ${isFullscreen ? "fixed inset-0 z-[99999] h-screen w-screen" : "flex-1 w-full h-full min-h-0"}`}>
      ...
      <div className="absolute top-2.5 right-3.5 z-30 flex items-center gap-1.5 bg-slate-900/90 hover:bg-slate-900 backdrop-blur-xl border border-blue-500/30 p-1.5 rounded-2xl shadow-2xl shadow-blue-950/40 transition-all">
        {/* Status Indicator */}
        <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-blue-950/60 border border-blue-500/30 text-blue-300 text-[11px] font-black mr-1">
          <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse" />
          <Truck className="h-3.5 w-3.5 text-blue-400" />
          <span>Sky CMR Express</span>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-800/80 p-0.5 rounded-xl border border-slate-700">
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-black transition-all cursor-pointer ${
              currentPath.includes("index.html") ? "bg-blue-600 text-white shadow-md font-extrabold" : "text-slate-300 hover:text-white hover:bg-slate-700/60"
            }`}
            title="Complete International CMR Editor"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-black transition-all cursor-pointer ${
              currentPath.includes("cmr_page1.html") ? "bg-blue-600 text-white shadow-md font-extrabold" : "text-slate-300 hover:text-white hover:bg-slate-700/60"
            }`}
            title="CMR Waybill Page 1"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-black transition-all cursor-pointer ${
              currentPath.includes("cmr_page2.html") ? "bg-blue-600 text-white shadow-md font-extrabold" : "text-slate-300 hover:text-white hover:bg-slate-700/60"
            }`}
            title="CMR Waybill Page 2"
          >
            <Layers className="h-3.5 w-3.5" />
        ...
        <button
          ...
          title="Open Sky CMR in a new tab"
        >
      ...
      {/* Loading Overlay */}
      {isLoading && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-slate-950/90 backdrop-blur-md transition-opacity">
          ...
          <span className="mt-4 text-sm font-black text-white tracking-wide">Loading Sky CMR International Waybill...</span>
          <span className="text-xs text-blue-400 font-semibold mt-1">Border Waybill & Consignment Note System</span>
        </div>
      )}
      ...
      <iframe
        ...
        title="Sky Ariana CMR International Consignment Note"
