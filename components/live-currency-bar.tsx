...
export function LiveCurrencyBar() {
  ...
  return (
    <div className="w-full bg-slate-900 border-b border-slate-800 text-slate-200 text-xs select-none no-print">
      <div className="max-w-[1880px] mx-auto px-3 py-1 flex flex-wrap items-center justify-between gap-2">
        {/* Ticker rates */}
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar py-0.5">
          <div className="flex items-center gap-1.5 text-amber-400 font-black shrink-0">
            <Globe className="w-3.5 h-3.5" />
            <span className="text-[11px] tracking-wide">FX BENCHMARK:</span>
          </div>

          <div className="flex items-center gap-2.5 shrink-0 font-mono text-[11px]">
            <span className="px-2 py-0.5 rounded bg-slate-800/80 border border-slate-700/60 font-semibold">
              1 USD = <span className="text-emerald-400 font-bold">{rates.AFN} AFN</span>
            </span>
            <span className="px-2 py-0.5 rounded bg-slate-800/80 border border-slate-700/60 font-semibold">
              1 USD = <span className="text-cyan-400 font-bold">{rates.IRR.toLocaleString()} IRR</span>
            </span>
            <span className="px-2 py-0.5 rounded bg-slate-800/80 border border-slate-700/60 font-semibold">
              1 USD = <span className="text-amber-400 font-bold">{rates.AED} AED</span>
            </span>
            <span className="px-2 py-0.5 rounded bg-slate-800/80 border border-slate-700/60 font-semibold">
              1 USD = <span className="text-purple-400 font-bold">{rates.EUR} EUR</span>
            </span>
            <span className="px-2 py-0.5 rounded bg-slate-800/80 border border-slate-700/60 font-semibold">
              1 USD = <span className="text-blue-400 font-bold">{rates.PKR} PKR</span>
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <div className="flex items-center gap-1 bg-slate-950/80 p-0.5 rounded-lg border border-slate-700/70">
            <input
              type="number"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              className="w-16 sm:w-20 bg-transparent text-white font-mono text-xs px-1.5 py-0.5 outline-none font-bold text-right"
              placeholder="Amount"
            />
            ...
            <select
              ...
              className="bg-slate-800 text-amber-300 text-[11px] font-bold px-1 py-0.5 rounded outline-none cursor-pointer"
            >
            ...
            <ArrowRightLeft className="w-3 h-3 text-slate-400 mx-0.5" />

            <select
              value={targetCurrency}
              onChange={(e) => setTargetCurrency(e.target.value as keyof LiveRates)}
              className="bg-slate-800 text-emerald-300 text-[11px] font-bold px-1 py-0.5 rounded outline-none cursor-pointer"
            >
              <option value="AFN">AFN</option>
              <option value="USD">USD</option>
              <option value="IRR">IRR</option>
            ...
            <span className="px-2 text-emerald-400 font-mono font-black text-xs">
            ...
            <Button
              ...
              className="h-6 w-6 rounded text-slate-400 hover:text-white hover:bg-slate-800 cursor-pointer"
          ...
          <Button
            ...
            className="h-6 px-1.5 text-[11px] text-slate-400 hover:text-white rounded cursor-pointer"
      ...
      {isExpanded && (
        <div className="bg-slate-950 border-t border-slate-800 p-2.5 animate-in fade-in duration-150">
          <div className="max-w-[1880px] mx-auto flex flex-wrap items-center justify-between gap-3">
