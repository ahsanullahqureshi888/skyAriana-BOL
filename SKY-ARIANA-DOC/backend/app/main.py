...
def health_check():
    ...
    try:
        with SessionLocal() as db:
            ...
            party_count = db.query(models.BusinessParty).count()
            product_count = db.query(models.Product).count()
    except Exception as e:
        db_status = f"degraded: {str(e)}"
        invoice_count = customer_count = party_count = product_count = 0

    return {
        "status": "online" if db_status == "healthy" else "degraded",
        "service": "Sky Ariana Logistics Backend API",
        "version": "2.4.0",
        "database": {
            "status": db_status,
            "metrics": {
